...
public class Network {
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i = 0; i < userCount; i++) {
            ...
            int currentUserMutal = user.countMutual(users[i]) ;
                if (currentUserMutal > maxInNumber) {
                    maxMutal = users[i];
                    maxInNumber = currentUserMutal;
                }
            }
        
        return maxMutal.getName();
    }
    ...
    public String mostPopularUser() {
        ...
        int max = -1;
        for (int i = 0; i < userCount; i++) {
            String curr = users[i].getName();
            int amount = followeeCount(curr);
            if(amount>max)
            {
                max = amount;
                popUser = curr;
    ...
    public String toString() {
        String fulltext = "Network:";
        for (int i = 0; i < userCount; i++) {
            fulltext += "\n";
           fulltext += users[i].toString();
        }
        return fulltext;
    }
}
